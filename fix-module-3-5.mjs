...
// Module 3.5 content - proper structure for sub-modules
const module35Content = {
  sections: [
    {
      title: "La oss først utforske annuiteter",
      type: "content",
...
**Forskjellen mellom annuitet og perpetuitet**
...
**Refleksjonsspørsmål:** Hvorfor tror du banker foretrekker annuitetslån fremfor serielån? Tenk på hvordan betalingene fordeler seg over tid og hvilken part som bærer renterisikoen.`
    }
  ]
};

async function fixModule35() {
  try {
    ...
    // Update sub-module 3.5 with proper JSONB content
    const { error: updateError } = await supabase
      .from('sub_modules')
      .update({ 
        content: module35Content, // Pass as object, not string
    ...
    if (updateError) {
      ...
      console.log('Successfully fixed sub-module 3.5 content structure');
    }
  } catch (error) {
