-- Update section 6.5 with additional insights and reflection questions
...
!insight Effisient front er ikke en teoretisk kuriositet - den brukes aktivt av profesjonelle kapitalforvaltere for å optimalisere porteföljer. Moderna programvare kan beregne effisient front for hundrevis av aktiva på sekunder.
...
!think Hvorfor skulle noen holde en portefølje som ligger under effisient front? I praksis skjer dette på grunn av transaksjonskostnader, skatter, regulering eller mangel på kunnskap om optimaliseringsmulighetene.
...
!insight CML-logikken forklarer hvorfor indeksfond er så populære: Markedsporteføljen M er ofte tilnærmet lik en bred markedsindeks, og CML viser at alle investorer (uavhengig av risikotoleranse) bør holde samme risikable portefølje.
...
!insight SML er grunnlaget for "stock picking": Hvis du finner en aksje som ligger over SML (positiv alpha), kan det være en god investeringsmulighet - men vær klar over at markedet kan være mer informert enn deg.
...
Kort sagt: **CML** viser den beste kombinasjonen av total risiko og avkastning når risikofri rente er tilgjengelig, mens **SML** fastsetter hvilken avkastning hver enkelt investering **må** tilby i forhold til sin $\beta$.'
            ...
            jsonb_build_object(
                ...
                'type', 'reflection',
                'content', '**Teori møter praksis:** Hvorfor tror du de fleste private investorer ikke optimaliserer porteføljene sine langs effisient front? Hvilke praktiske hindringer finnes?

**Markedsporteføljen:** Hvis "markedsporteføljen" skal inneholde alle investérbare aktiva i verden, hvorfor bruker vi ofte bare aksjeindekser som tilnærming? Hva mangler vi?

**CML vs. SML:** Du har en aksje med β = 0,8 som gir samme avkastning som markedet. Ligger denne aksjen på CML, SML, begge, eller ingen av dem? Forklar resonnementet ditt.

**Investoradferd:** Hvis alle rasjonelle investorer skal holde samme risikable portefølje (M) kombinert med risikofri rente, hvorfor ser vi så stor variasjon i porteføljer i virkeligheden?'
            )
        )
    ),
    updated_at = now()
