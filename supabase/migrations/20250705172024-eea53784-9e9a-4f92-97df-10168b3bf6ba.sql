-- Update section 6.6 with strengths and limitations of CAPM
...
1. **Effisient front:** Bygg alle mulige porteföljer og finn øvre venstrekant i $(E[R],\sigma)$-planet.
...
• **Operasjonell** – Krever bare β-estimat og markedsdata, som gjør den praktisk anvendelig
...
• **Empiriske avvik** – Systematiske avvik fra modellen (f.eks. verdi- og størrelseseffekter) er dokumentert

• **Total risiko kan spille en rolle** – For investorer med ikke-diversifiserte porteföljer eller likviditetsbegrensninger kan total risiko være relevant'
            )
        )
    ),
    updated_at = now()
